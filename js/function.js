...
function searchList(dataName,url){
    ...
    for(var i=0; i<dataName.length; i++){
        search_list+='<a href="'+url+'" class="item search_link1"><div class="search_shell">'
            ...
            +'</div>'
            +'<div class="search_item search_item_grey">'
...
function airlineDetail(url){
    ...
    for(var i=0; i<data.length; i++){
        list+='<div class="item"><div class="row">'
            ...
            +'</div>'
            +'<div class="col col-20"><button class="button button-assertive fr advance">预订</button></div>'
            +'</div></div>'
    }
    list+='</div>';
 ...
 function output(){
          var city='<div class="city"><div class="bar bar-header bar-dark"><a class="button icon-left ion-ios-arrow-back button-clear button-light"></a><h1 class="title">选择城市</h1></div><div class="list list-inset" style="margin-top:55px; margin-bottom:10px;border-radius:10px;"><label class="item item-input icon-search-left"><i class="icon icon-search placeholder-icon"><img src="./images/icon_search.png" /></i><input type="text" placeholder="请输入城市中文、拼音、三字码"></label></div><div class="citylist"></div><div class="catalog"></div></div>';
            $(city).appendTo('body');
...
function orderList(){
    ...
    for(var i=0; i<orderlistdata.length; i++){
        orderlist_box+='<div class="list card">'
                    ...
                    orderlist_box+='<a class="item" href="#">'
